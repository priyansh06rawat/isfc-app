/**
 * get-lead-rest-service.js
 * 
 * Fetches the complete ApexClass body of LeadRestService and writes it to salesforce/LeadRestService.cls.
 */
...
async function main() {
  ...
  console.log('Querying LeadRestService from Tooling API...');
  const q = encodeURIComponent("SELECT Body FROM ApexClass WHERE Name = 'LeadRestService'");
  const res = await request(
  ...
  if (res.status !== 200) {
    console.error('Failed to query Apex class:', res.body);
    return;
  }
  
  const records = res.body.records || [];
  if (records.length === 0) {
    console.error('LeadRestService class not found in Salesforce org.');
    return;
  }
  
  const body = records[0].Body;
  const outputPath = path.join(__dirname, '..', 'salesforce', 'LeadRestService.cls');
  fs.writeFileSync(outputPath, body);
  console.log(`✅ Saved LeadRestService body to ${outputPath}`);
}
